...
bool check(vector<pair<double, double>>& people, double t){

    pair<double, double> c = {-1e18, 1e18};
    for(auto &it: people){
        double x = it.first;
        double speed = it.second;
        pair<double, double> range = {x - (t * speed), x + (t * speed)};
        c = {max(c.first, range.first), min(c.second, range.second)};
        if(c. first > c.second){
            return false;
        }
    }
